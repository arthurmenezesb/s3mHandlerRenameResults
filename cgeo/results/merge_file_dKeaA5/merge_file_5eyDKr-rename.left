...
public class cgeoapplication extends Application {
    ...
    private final Map<UUID, cgSearch> searches = new HashMap<UUID, cgSearch>();

    // information about searches
    private final Map<String, cgCache> cachesCache = new HashMap<String, cgCache>();

    // caching caches into memory
    public boolean firstRun = true;

    // c:geo is just launched
    public boolean warnedLanguage = false;

    // user was warned about different language settings on geocaching.com
    ...
    private boolean storeWithMerge(cgCache cache, boolean forceSave) {
        ...
        if (// if for offline, do not merge
        forceSave || (oldCache = storage.loadCache(cache.geocode, cache.guid, false, true, true, true, true, true)) != null) {
