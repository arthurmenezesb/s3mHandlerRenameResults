...
public final class FileUtils {
        ...
        if (ArrayUtils.isNotEmpty(files)) {
            for (File file : files) {
                ...
                if (file.isFile()) {
                    ...
                    // go deeper
                    listDir(result, file, chooser, feedBackHandler);
                }
            }
        }
    }

    public abstract static class FileSelector {
    ...
    public static boolean mkdirs(File file) {
        // mkdirs returns false on existing directories
        final boolean success = file.mkdirs() || file.isDirectory();
        if (!success) {
