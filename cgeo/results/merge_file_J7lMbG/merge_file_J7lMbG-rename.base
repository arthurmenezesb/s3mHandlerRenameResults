...
public class cgeopoint extends Activity {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (menuItem == 1) {
            ...
            base.runExternalMap(cgBase.mapAppLocus, activity, res, warning, tracker, coords.get(0), // locus
            coords.get(// locus
            1));
            return true;
        } else if (menuItem == 21) {
            base.runExternalMap(cgBase.mapAppRmaps, activity, res, warning, tracker, coords.get(0), // rmaps
            coords.get(// rmaps
            1));
            return true;
        } else if (menuItem == 23) {
            base.runExternalMap(cgBase.mapAppAny, activity, res, warning, tracker, coords.get(0), // rmaps
            coords.get(// rmaps
            1));
            return true;
        }
    ...
    private ArrayList<Double> getDestination() {
        ...
        if (bearingText != null && bearingText.length() > 0 && distanceText != null && distanceText.length() > 0) {
            ...
            distance = // km
            null;
            final Pattern patternA = Pattern.compile("^([0-9\\.\\,]+)[ ]*m$", // m
            Pattern.CASE_INSENSITIVE);
            final Pattern patternB = Pattern.compile("^([0-9\\.\\,]+)[ ]*km$", // km
            Pattern.CASE_INSENSITIVE);
            // ft - 0.3048m
    ...
    public void goManual(View view) {
        try {
            AppManualReaderClient.openManual("c-geo", "c:geo-navigate-any", activity, "http://cgeo.carnero.cc/manual/");
        } catch (Exception e) {
        }
    }
}
