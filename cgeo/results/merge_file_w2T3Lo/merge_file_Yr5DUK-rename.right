...
public abstract class GCParser {
    ...
    private static final SynchronizedDateFormat dateTbIn1 = new SynchronizedDateFormat("EEEEE, dd MMMMM yyyy", Locale.ENGLISH);

    // Saturday, 28 March 2009
    private static final SynchronizedDateFormat dateTbIn2 = new SynchronizedDateFormat("EEEEE, MMMMM dd, yyyy", Locale.ENGLISH);
    ...
    public static SearchResult searchByCoords(@NonNull final Geopoint coords, final CacheType cacheType, final boolean showCaptcha, RecaptchaReceiver recaptchaReceiver) {
    ...
    public static SearchResult searchByKeyword(@NonNull final String keyword, final CacheType cacheType, final boolean showCaptcha, RecaptchaReceiver recaptchaReceiver) {
    ...
    public static ImmutablePair<StatusCode, String> postLog(final String geocode, final String cacheid, final String[] viewstates, final LogType logType, final int year, final int month, final int day, final String log, final List<TrackableLog> trackables) {
        if (GCLogin.isEmpty(viewstates)) {
        ...
        // windows' eol and remove leading and trailing whitespaces
        ...
        if (trackables != null && !trackables.isEmpty()) {
            ...
            // selected trackables
            params.put(// selected trackables
            "ctl00$ContentBody$LogBookPanel1$uxTrackables$hdnSelectedActions", // selected trackables
            hdnSelected.toString(), "ctl00$ContentBody$LogBookPanel1$uxTrackables$hdnCurrentFilter", "");
        }
     ...
     */
    public static StatusCode postLogTrackable(final String tbid, final String trackingCode, final String[] viewstates, final LogType logType, final int year, final int month, final int day, final String log) {
        if (GCLogin.isEmpty(viewstates)) {
    ...
    static boolean removeFromWatchlist(final Geocache cache) {
        ...
        // on watch list (=error) / not on watch list
        return !guidOnPage;
    }
    ...
    private static Observable<LogEntry> getLogs(final String page, final Logs logType) {
        return Observable.defer(new Func0<Observable<LogEntry>>() {
            ...
            public Observable<LogEntry> call() {
                ...
                final Parameters params = new Parameters("tkn", userToken, "idx", "1", "num", String.valueOf(GCConstants.NUMBER_OF_LOGS), "decrypt", "true");
                if (logType != Logs.ALL) {
    ...
    private static void getExtraOnlineInfo(final Geocache cache, final String page, final CancellableHandler handler) {
        ...
        if (Settings.isFriendLogsWanted()) {
            ...
            specialLogs = Observable.merge(getLogs(page, Logs.FRIENDS), getLogs(page, Logs.OWN));
        } else {
            CancellableHandler.sendLoadProgressDetail(handler, R.string.cache_dialog_loading_details_status_logs);
    ...
    public static boolean uploadPersonalNote(Geocache cache) {
        ...
        try {
            final JSONObject jo = new JSONObject().put("dto", (new JSONObject().put("et", StringUtils.defaultString(cache.getPersonalNote())).put("ut", userToken)));
