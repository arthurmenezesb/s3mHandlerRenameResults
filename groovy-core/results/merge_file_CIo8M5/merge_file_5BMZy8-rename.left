...
public class StaticTypeCheckingVisitor extends ClassCodeVisitorSupport {
    ...
    static {
        ...
        CLOSURE_CALL_ONE_ARG = CLOSURE_TYPE.getDeclaredMethod("call", new Parameter[] { new Parameter(OBJECT_TYPE, "arg") });
        CLOSURE_CALL_VARGS = CLOSURE_TYPE.getDeclaredMethod("call", new Parameter[] { new Parameter(OBJECT_TYPE.makeArray(), "args") });
    }
    ...
    public void visitVariableExpression(VariableExpression vexp) {
        ...
        if (vexp != VariableExpression.THIS_EXPRESSION && vexp != VariableExpression.SUPER_EXPRESSION) {
            ...
            if (vexp.getName().equals("super"))
                storeType(vexp, classNode.getSuperClass());
        }
    ...
    public void visitPropertyExpression(final PropertyExpression pexp) {
        ...
        if (!existsProperty(pexp, true)) {
            ...
            addStaticTypeError("No such property: " + pexp.getPropertyAsString() + " for class: " + findCurrentInstanceOfClass(objectExpression, objectExpression.getType()), pexp);
        }
    }
    ...
    private void typeCheckAssignment(final BinaryExpression assignmentExpression, final Expression leftExpression, final ClassNode leftExpressionType, final Expression rightExpression, final ClassNode inferredRightExpressionType) {
    ...
    private void addClosureReturnType(ClassNode returnType) {
        if (closureReturnTypes == null)
            closureReturnTypes = new LinkedList<ClassNode>();
    ...
    public void visitMethod(final MethodNode node) {
        ...
        if (alreadyVisitedMethods.contains(node))
            return;
        alreadyVisitedMethods.add(node);
        // may use this visitor without the annotation being explicitely set
        if (!methodsToBeVisited.isEmpty() && !methodsToBeVisited.contains(node))
            return;
    ...
    public void visitMethodCallExpression(MethodCallExpression call) {
        ...
        if (isWithCall) {
            // must be added first in the list
        ...
        try {
            ...
            if (isCallOnClosure) {
                ...
                if (!withReceiverList.isEmpty())
                    receivers.addAll(withReceiverList);
                    ...
                    if (mn.size() == 1) {
                        ...
                        if (objectExpression instanceof VariableExpression) {
                            ...
                            if (var.isClosureSharedVariable())
                                secondPassExpressions.add(call);
                        }
                    } else {
    ...
    private ClassNode getResultType(ClassNode left, int op, ClassNode right, BinaryExpression expr) {
        ...
        if (op == ASSIGN) {
            ...
            if (leftExpression instanceof VariableExpression) {
                ...
                if (STRING_TYPE.equals(initialType) || CLASS_Type.equals(initialType) || Boolean_TYPE.equals(initialType)) {
                    return initialType;
                }
            }
            return right;
            ...
            return Matcher_TYPE;
        } else // for primitives and their wrapper we use a fixed table here
        if (isNumberType(leftRedirect) && isNumberType(rightRedirect)) {
            if (isOperationInGroup(op)) {
                ...
                if (isLongCategory(leftRedirect) && isLongCategory(rightRedirect))
                    return long_TYPE;
                if (isFloat(leftRedirect) && isFloat(rightRedirect))
                    return float_TYPE;
                if (isDouble(leftRedirect) && isDouble(rightRedirect))
                    return double_TYPE;
                ...
                if (isLongCategory(leftRedirect) && isLongCategory(rightRedirect))
                    return Long_TYPE;
                if (isBigIntCategory(leftRedirect) && isBigIntCategory(rightRedirect))
    ...
    private static ClassNode getGroupOperationResultType(ClassNode a, ClassNode b) {
        ...
        if (isBigDecCategory(a) && isBigDecCategory(b))
            return BigDecimal_TYPE;
        if (BigDecimal_TYPE.equals(a) || BigDecimal_TYPE.equals(b))
            return BigDecimal_TYPE;
        ...
        if (double_TYPE.equals(a) || double_TYPE.equals(b))
            return double_TYPE;
        if (Double_TYPE.equals(a) || Double_TYPE.equals(b))
            return Double_TYPE;
        if (float_TYPE.equals(a) || float_TYPE.equals(b))
            return float_TYPE;
        if (Float_TYPE.equals(a) || Float_TYPE.equals(b))
            return Float_TYPE;
        if (long_TYPE.equals(a) || long_TYPE.equals(b))
            return long_TYPE;
        if (Long_TYPE.equals(a) || Long_TYPE.equals(b))
            return Long_TYPE;
        if (int_TYPE.equals(a) || int_TYPE.equals(b))
            return int_TYPE;
        if (Integer_TYPE.equals(a) || Integer_TYPE.equals(b))
            return Integer_TYPE;
        if (short_TYPE.equals(a) || short_TYPE.equals(b))
            return short_TYPE;
        if (Short_TYPE.equals(a) || Short_TYPE.equals(b))
            return Short_TYPE;
        if (byte_TYPE.equals(a) || byte_TYPE.equals(b))
            return byte_TYPE;
        if (Byte_TYPE.equals(a) || Byte_TYPE.equals(b))
            return Byte_TYPE;
        if (char_TYPE.equals(a) || char_TYPE.equals(b))
            return char_TYPE;
        if (Character_TYPE.equals(a) || Character_TYPE.equals(b))
        ...
        for (MethodNode m : methods) {
            ...
            if (params.length == args.length) {
                ...
                if (dist >= 0 && !receiver.equals(m.getDeclaringClass()))
                    dist += getDistance(receiver, m.getDeclaringClass());
    ...
    private void typeCheckMethodsWithGenerics(ClassNode receiver, ClassNode[] arguments, List<MethodNode> candidateMethods, Expression location) {
        ...
        for (MethodNode method : candidateMethods) {
            ...
            if (!implementsInterfaceOrIsSubclassOf(receiver, methodNodeReceiver) || !methodNodeReceiver.isUsingGenerics())
                continue;
