...
public class StaticTypeCheckingVisitor extends ClassCodeVisitorSupport {
    ...
    public void visitStaticMethodCallExpression(final StaticMethodCallExpression call) {
        ...
        try {
            ...
            for (ClassNode currentReceiver : receivers) {
                ...
                if (!mn.isEmpty()) {
                    if (mn.size() == 1)
                        typeCheckMethodsWithGenerics(currentReceiver, args, mn.get(0), call);
    ...
    private boolean isClosureCall(final String name, final Expression objectExpression, final Expression arguments) {
        ...
        if (objectExpression == VariableExpression.THIS_EXPRESSION) {
            ...
            if (!"call".equals(name) && !"doCall".equals(name))
                return false;
        }
                ...
                if (name.startsWith("set")) {
                    ...
                    if (property != null) {
                        ...
                        if (implementsInterfaceOrIsSubclassOf(args[0], type)) {
                            return Collections.singletonList(new MethodNode(name, Opcodes.ACC_PUBLIC, VOID_TYPE, new Parameter[] { new Parameter(type, "arg") }, ClassNode.EMPTY_ARRAY, EmptyStatement.INSTANCE));
                        }
                    }
                }
            }
    ...
    private void typeCheckMethodsWithGenerics(ClassNode receiver, ClassNode[] arguments, MethodNode candidateMethod, Expression location) {
        if (!isUsingGenericsOrIsArrayUsingGenerics(receiver))
            return;
    ...
    protected void addError(final String msg, final ASTNode expr) {
        ...
        errorCollector.addErrorAndContinue(new SyntaxErrorMessage(new SyntaxException(msg + '\n', line, col), source));
    }
