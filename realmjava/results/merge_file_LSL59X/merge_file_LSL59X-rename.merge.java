...
public class NotificationsTest {
    ...
    public void closeClearingHandlerMessages() throws InterruptedException, TimeoutException, ExecutionException {
        ...
        Future<Boolean> future = executorService.submit(new Callable<Boolean>() {
            ...
            @Override
            public Boolean call() throws Exception {
                // Fake background thread with a looper, eg. a IntentService.
                Looper.prepare();
                Realm realm = Realm.getInstance(realmConfig);
                ...
                // Close native resources + associated handlers.
                realm.close();
    ...
    public void looperThreadQuitsLooperEarly() throws InterruptedException {
        ...
        RealmLog.add(logger);
        realm.beginTransaction();
        // If the Handler on the background is notified it will trigger a Log warning.
        realm.commitTransaction();
    ...
    @Test
    @RunTestInLooperThread(/*step1*/
    before = PopulateOneAllTypes.class)
    ...
    public void callingOrdersOfListeners() {
        ...
        final RealmChangeListener<RealmResults<AllTypes>> syncedResultsListener = new RealmChangeListener<RealmResults<AllTypes>>() {
            ...
            public void onChange(RealmResults<AllTypes> results) {
                switch(asyncResultCallback.incrementAndGet()) {
                    ...
                    case 2:
                        ...
                        assertEquals(1, results.size());
                        // If syncResults is not in sync yet, this will fail.
    ...
    public void accessingSyncRealmResultsInsideAnotherResultListener() {
        ...
        syncResults1.addChangeListener(new RealmChangeListener<RealmResults<AllTypes>>() {
            ...
            public void onChange(RealmResults<AllTypes> element) {
                ...
                // If syncResults2 is not in sync yet, this will fail.
