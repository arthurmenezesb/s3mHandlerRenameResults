...
public class RealmSourceCodeGenerator {
    ...
    public boolean generate() throws Exception {
        ...
        while (it.hasNext()) {
            ...
            if (_fieldTable.length() > 0)
                _fieldTable += " ,";
            _fieldTable += "\"" + k + "\"";
            if (_typeTable.length() > 0)
                _typeTable += " ,";
            if (e.asType().toString().compareTo("java.lang.String") == 0) {
                _typeTable += ColumnType.STRING.getValue();
            } else if (e.asType().toString().compareTo("int") == 0 || e.asType().toString().compareTo("long") == 0 || e.asType().toString().compareTo("java.lang.Integer") == 0 || e.asType().toString().compareTo("java.lang.Long") == 0) {
            ...
            } else //			else if (e.asType().equals(byte[].class) )
            //			{
