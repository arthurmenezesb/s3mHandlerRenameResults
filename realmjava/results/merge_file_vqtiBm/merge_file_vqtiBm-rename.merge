...
public final class SharedRealm implements Closeable {
    ...
    public enum SchemaMode {
        ...
        SCHEMA_MODE_AUTOMATIC(SCHEMA_MODE_VALUE_AUTOMATIC), SCHEMA_MODE_READONLY(SCHEMA_MODE_VALUE_READONLY), SCHEMA_MODE_RESET_FILE(SCHEMA_MODE_VALUE_RESET_FILE), SCHEMA_MODE_ADDITIVE(SCHEMA_MODE_VALUE_ADDITIVE), SCHEMA_MODE_MANUAL(SCHEMA_MODE_VALUE_MANUAL);
    ...
    public static SharedRealm getInstance(RealmConfiguration config, RealmNotifier realmNotifier) {
        ...
        // Handled in Java currently
        boolean enable_caching = false;
        // TODO Double negatives :/
        boolean disableFormatUpgrade = false;
        boolean autoChangeNotifications = true;
        long nativeConfigPtr = nativeCreateConfig(config.getPath(), config.getEncryptionKey(), rosServerUrl != null ? SchemaMode.SCHEMA_MODE_ADDITIVE.getNativeValue() : SchemaMode.SCHEMA_MODE_MANUAL.getNativeValue(), config.getDurability() == Durability.MEM_ONLY, enable_caching, disableFormatUpgrade, autoChangeNotifications, rosServerUrl, rosUserToken);
        try {
    ...
    public static SharedRealm getInstance(RealmConfiguration config, RealmNotifier realmNotifier, SchemaVersionListener schemaVersionListener) {
        long nativeConfigPtr = nativeCreateConfig(config.getPath(), config.getEncryptionKey(), SchemaMode.SCHEMA_MODE_MANUAL.value, config.getDurability() == Durability.MEM_ONLY, false, false, true);
        try {
            return new SharedRealm(nativeGetSharedRealm(nativeConfigPtr, realmNotifier), config, realmNotifier, schemaVersionListener);
        } finally {
            nativeCloseConfig(nativeConfigPtr);
        }
    }
    ...
    private static native long nativeCreateConfig(String realmPath, byte[] key, byte schemaMode, boolean inMemory, boolean cache, boolean disableFormatUpgrade, boolean autoChangeNotification, String syncServerURL, String syncUserToken);
