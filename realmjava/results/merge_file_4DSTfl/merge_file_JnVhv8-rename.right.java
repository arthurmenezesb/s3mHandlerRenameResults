...
public class RealmQuery<E extends RealmModel> {
            ...
            public Long call() throws Exception {
                if (!Thread.currentThread().isInterrupted()) {
                    ...
                    try {
                        ...
                        long handoverTableViewPointer = TableQuery.findDistinctWithHandover(sharedRealm, handoverQueryPointer, columnIndex);
                        ...
                        closeSharedRealmAndSendEventToNotifier(sharedRealm, weakNotifier, QueryUpdateTask.NotifyEvent.COMPLETE_ASYNC_RESULTS, result);
                        return handoverTableViewPointer;
                    } catch (Throwable e) {
                        RealmLog.error(e);
                        closeSharedRealmAndSendEventToNotifier(sharedRealm, weakNotifier, QueryUpdateTask.NotifyEvent.THROW_BACKGROUND_EXCEPTION, e);
                    } finally {
            ...
            public Long call() throws Exception {
                if (!Thread.currentThread().isInterrupted()) {
                    ...
                    try {
                        ...
                        // to import it.
                        long handoverTableViewPointer = TableQuery.findAllWithHandover(sharedRealm, handoverQueryPointer);
                        QueryUpdateTask.Result result = QueryUpdateTask.Result.newRealmResultsResponse();
                        result.updatedTableViews.put(weakRealmResults, handoverTableViewPointer);
                        result.versionID = sharedRealm.getVersionID();
                        closeSharedRealmAndSendEventToNotifier(sharedRealm, weakNotifier, QueryUpdateTask.NotifyEvent.COMPLETE_ASYNC_RESULTS, result);
                        ...
                        RealmLog.debug("findAllAsync handover could not complete due to a BadVersionException. " + "Retry is scheduled by a REALM_CHANGED event.");
                    } catch (Throwable e) {
                        RealmLog.error(e);
                        closeSharedRealmAndSendEventToNotifier(sharedRealm, weakNotifier, QueryUpdateTask.NotifyEvent.THROW_BACKGROUND_EXCEPTION, e);
                    } finally {
            ...
            public Long call() throws Exception {
                if (!Thread.currentThread().isInterrupted()) {
                    ...
                    try {
                        ...
                        long columnIndex = getColumnIndexForSort(fieldName);
                        // Runs the query & handover the table view for the caller thread.
                        long handoverTableViewPointer = TableQuery.findAllSortedWithHandover(sharedRealm, handoverQueryPointer, columnIndex, sortOrder);
                        QueryUpdateTask.Result result = QueryUpdateTask.Result.newRealmResultsResponse();
                        result.updatedTableViews.put(weakRealmResults, handoverTableViewPointer);
                        result.versionID = sharedRealm.getVersionID();
                        closeSharedRealmAndSendEventToNotifier(sharedRealm, weakNotifier, QueryUpdateTask.NotifyEvent.COMPLETE_ASYNC_RESULTS, result);
                        ...
                        RealmLog.debug("findAllSortedAsync handover could not complete due to a BadVersionException. " + "Retry is scheduled by a REALM_CHANGED event.");
                    } catch (Throwable e) {
                        RealmLog.error(e);
                        closeSharedRealmAndSendEventToNotifier(sharedRealm, weakNotifier, QueryUpdateTask.NotifyEvent.THROW_BACKGROUND_EXCEPTION, e);
                    } finally {
                ...
                public Long call() throws Exception {
                    if (!Thread.currentThread().isInterrupted()) {
                        ...
                        try {
                            sharedRealm = SharedRealm.getInstance(realmConfiguration);
                            // Runs the query & handover the table view for the caller thread.
                            long handoverTableViewPointer = TableQuery.findAllMultiSortedWithHandover(sharedRealm, handoverQueryPointer, indices, sortOrders);
                            QueryUpdateTask.Result result = QueryUpdateTask.Result.newRealmResultsResponse();
                            result.updatedTableViews.put(weakRealmResults, handoverTableViewPointer);
                            result.versionID = sharedRealm.getVersionID();
                            closeSharedRealmAndSendEventToNotifier(sharedRealm, weakNotifier, QueryUpdateTask.NotifyEvent.COMPLETE_ASYNC_RESULTS, result);
    ...
    public E findFirstAsync() {
        ...
        if (isDynamicQuery()) {
            ...
            result = realm.getConfiguration().getSchemaMediator().newInstance(clazz, realm, Row.EMPTY_ROW, realm.getSchema().getColumnInfo(clazz), false, Collections.<String>emptyList());
        }
            ...
            public Long call() throws Exception {
                if (!Thread.currentThread().isInterrupted()) {
                    ...
                    try {
                        ...
                        result.versionID = sharedRealm.getVersionID();
                        closeSharedRealmAndSendEventToNotifier(sharedRealm, weakNotifier, QueryUpdateTask.NotifyEvent.COMPLETE_ASYNC_OBJECT, result);
    ...
    private void checkSortParameters(String fieldNames[], final Sort[] sortOrders) {
        if (fieldNames == null) {
            ...
            throw new IllegalArgumentException(String.format(Locale.ENGLISH, "Number of field names (%d) and sort orders (%d) does not match.", fieldNames.length, sortOrders.length));
        }
    }
    ...
    private WeakReference<RealmNotifier> getWeakReferenceNotifier() {
        ...
        // Uses caller Realm's Looper.
        return new WeakReference<RealmNotifier>(realm.sharedRealm.realmNotifier);
    }
    ...
    private void closeSharedRealmAndSendEventToNotifier(SharedRealm sharedRealm, WeakReference<RealmNotifier> weakNotifier, QueryUpdateTask.NotifyEvent event, Object obj) {
        if (sharedRealm != null) {
            sharedRealm.close();
        }
