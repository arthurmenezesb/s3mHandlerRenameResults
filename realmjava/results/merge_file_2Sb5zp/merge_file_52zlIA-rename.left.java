...
public final class Realm implements Closeable {
    ...
    protected void finalize() throws Throwable {
        if (sharedGroup != null) {
            RealmLog.w("Remember to call close() on all Realm instances. " + "Realm " + path + " is being finalized without being closed, " + "this can lead to running out of native memory.");
        }
        super.finalize();
    }
    ...
    public static Realm getInstance(File writeableFolder) {
        return create(new RealmConfiguration.Builder(writeableFolder).name(DEFAULT_REALM_NAME).create());
    }
    ...
    public static Realm getInstance(File writeableFolder, String fileName) {
        return create(new RealmConfiguration.Builder(writeableFolder).name(fileName).create());
    }
    ...
    public static Realm getInstance(File writeableFolder, byte[] key) {
        return create(new RealmConfiguration.Builder(writeableFolder).name(DEFAULT_REALM_NAME).encryptionKey(key).create());
    }
    ...
    public static Realm getInstance(File writeableFolder, String fileName, byte[] key) {
        return create(new RealmConfiguration.Builder(writeableFolder).name(fileName).encryptionKey(key).create());
    }
    ...
    public static synchronized void migrateRealmAtPath(String realmPath, byte[] key, RealmMigration migration, boolean autoUpdate) {
        ...
        migrateRealm(new RealmConfiguration.Builder(file.getParentFile()).name(file.getName()).encryptionKey(key).migration(migration).create());
    }
    ...
    public static boolean deleteRealmFile(Context context, String fileName) {
        return deleteRealmFile(new RealmConfiguration.Builder(context).name(fileName).create());
    }
    ...
    public static boolean compactRealmFile(RealmConfiguration configuration) {
        ...
        File tmpFile = new File(configuration.getFileDir(), String.valueOf(System.currentTimeMillis()) + UUID.randomUUID() + ".realm");
        Realm realm = null;
        try {
