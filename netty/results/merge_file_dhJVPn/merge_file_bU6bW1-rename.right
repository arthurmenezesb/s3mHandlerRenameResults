...
public abstract class ReplayingDecoder<T extends Enum<T>> extends FrameDecoder {
    ...
    protected void checkpoint() {
        ...
        if (cumulation != null) {
            ...
            // buffer not available (already cleaned up)
            checkpoint = -1;
        }
    }
        ...
        if (cumulation == null) {
            ...
            if (input.readable()) {
                ...
                if (checkpoint > 0) {
                    ...
                    cumulation = this.cumulation = newCumulationBuffer(ctx, bytesToPreserve);
                    cumulation.writeBytes(input, checkpoint, bytesToPreserve);
                } else if (checkpoint == 0) {
                    cumulation = this.cumulation = newCumulationBuffer(ctx, inputSize);
                    cumulation.writeBytes(input, oldReaderIndex, inputSize);
                    cumulation.readerIndex(input.readerIndex());
                } else {
                    cumulation = this.cumulation = newCumulationBuffer(ctx, input.readableBytes());
