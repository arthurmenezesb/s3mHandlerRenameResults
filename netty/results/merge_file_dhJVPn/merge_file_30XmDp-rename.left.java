...
public abstract class ReplayingDecoder<O, S extends Enum<S>> extends StreamToMessageDecoder<O> {
    ...
    @Override
    public ChannelBufferHolder<Byte> newInboundBuffer(ChannelInboundHandlerContext<Byte> ctx) throws Exception {
        if (inUse) {
            throw new IllegalStateException(ReplayingDecoder.class.getSimpleName() + " cannot be shared.");
        }
        inUse = true;
        return in;
    }
