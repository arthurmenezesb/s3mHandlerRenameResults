...
public class SpdyFrameDecoder extends FrameDecoder {
    ...
    private final SpdyHeaderBlockDecompressor headerBlockDecompressor = SpdyHeaderBlockDecompressor.newInstance();
    ...
    protected Object decode(ChannelHandlerContext ctx, Channel channel, ChannelBuffer buffer) throws Exception {
        ...
        if (frameLength > maxFrameSize) {
            throw new SpdyProtocolException("Frame length exceeds " + maxFrameSize + ": " + frameLength);
        }
    ...
    private void decodeHeaderBlock(SpdyHeaderBlock headerFrame, ChannelBuffer headerBlock) throws Exception {
        if ((headerBlock.readableBytes() == 2) && (headerBlock.getShort(headerBlock.readerIndex()) == 0)) {
            return;
        }
        ...
        for (int i = 0; i < numEntries; i++) {
            ...
            if (headerSize > maxHeaderSize) {
                throw new SpdyProtocolException("Header block exceeds " + maxHeaderSize);
            }
